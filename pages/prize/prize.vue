<template>
  <view>
    ...
    <view class="btns">
      <view
        :class="['default_btn', { active: current == 0 }]"
        @click="changeType(0)"
        >全部</view
      >
      <view
        :class="['default_btn', { active: current == 1 }]"
        @click="changeType(1)"
        >实物</view
      >
      <view
        :class="['default_btn', { active: current == 2 }]"
        @click="changeType(2)"
        >优惠券</view
      >
    </view>
    <view class="goods_list" v-if="userPrizeList.length > 0">
      <view
        v-for="item in userPrizeList"
        :class="['item_box', { item_gray: item.is_verify != 0 }]"
        :key="item"
      >
        <view class="item_top">
...
.goods_list {
  ...
  .item_box {
    ...
    &.item_gray {
      background: url(https://static.roi-cloud.com/base/icon_accpet_prize.png)
        no-repeat #fff;
